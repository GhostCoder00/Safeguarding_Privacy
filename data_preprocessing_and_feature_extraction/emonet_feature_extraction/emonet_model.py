...
class HourGlass(nn.Module):
    ...
    def _forward(self, level: int, inp: torch.Tensor)-> torch.Tensor:
        ...
        if up1.size(dim=2) > up2.size(dim=2):
            up2 = F.pad(up2, pad=(0, up1.size(dim=1)-up2.size(dim=1), up1.size(dim=2)-up2.size(dim=2), up1.size(dim=3)-up2.size(dim=3)), mode='replicate')
...
class EmoNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor, reset_smoothing: bool = False)-> dict:
        ...
        #final_features_out = torch.flatten(final_features)
        ...
        final_features_out = torch.flatten(final_features)
        final_features = final_features_out[::2]
        final_features = self.emo_fc_2(torch.unsqueeze(final_features, 0))
        ...
        return {'features': final_features_out,'heatmap': tmp_out, 'expression': final_features[:,:-2], 'valence': final_features[:,-2], 'arousal':final_features[:,-1]}
