...
def federated_learning(args: object, train_clients: list[object], test_clients: list[object], global_model: torch.nn.Module) -> None:
    ...
    global_optim = args.global_optim(global_model.parameters(), lr = pow(10, args.global_lr))
    logits_optim = args.logits_optim(global_model.logits.parameters(), lr = pow(10, args.logits_lr))
    
    # train-test split on server level
    ...
    global_train_loader  = torch.utils.data.DataLoader(global_train_dataset, batch_size = args.batch_size, shuffle = False)
    global_test_loader   = torch.utils.data.DataLoader(global_test_dataset , batch_size = args.batch_size, shuffle = False)
    ...
    for current_global_epoch in tqdm.tqdm(range(args.global_epoch)):
        ...
        client_weights = [len(c.dataset) for c in update_clients]
        ...
        eval(args.fed_agg)(global_model, client_models, client_weights, # basic FL parameters
                           global_optim, # for FedOpt (FedAdam)
                           logits_optim, # for FedAwS and TurboSVM
                           current_global_epoch, args.global_epoch) # for TurboSVM
...
def TurboSVM(global_model: torch.nn.Module, 
    ...
    # randomly select classes, whose embeddings will be updated
